...
class AadhaarOCR:
    def __init__(self, use_angle_cls=True, lang="en"):
        ...
        if self.ocr is None:
            raise RuntimeError(
                f"Failed to initialize PaddleOCR with any configuration. Last error: {last_error}"
            )
    ...
    def extract_text_with_paddle(self, image, confidence_threshold=0.5):
        ...
        try:
            for item_container in raw_result:
                ...
                if (
                    isinstance(item_container, dict)
                    and "rec_texts" in item_container
                    and "rec_scores" in item_container
                ):
                    for text, score in zip(
                        item_container["rec_texts"], item_container["rec_scores"]
                    ):
                ...
                # Handle OCRResult objects
                    ...
                    if hasattr(item_container, "rec_texts") and hasattr(
                        item_container, "rec_scores"
                    ):
                        for text, score in zip(
                            item_container.rec_texts, item_container.rec_scores
                        ):
                ...
                # Handle list format (bbox, text, confidence)
                    ...
                    for bbox_text_conf in item_container:
                        if (
                            isinstance(bbox_text_conf, list)
                            and len(bbox_text_conf) >= 2
                        ):
                            if (
                                isinstance(bbox_text_conf[1], list)
                                and len(bbox_text_conf[1]) >= 2
                            ):
            ...
            try:
                ...
                for line in potential_lines:
                    if len(line.strip()) > 3 and not any(
                        k in line.lower() for k in ["bbox", "score", "conf"]
                    ):
    ...
    def extract_name(self, text):
        ...
        for i, line in enumerate(lines):
            ...
            if (
                "government of india" in line_lower
                or "unique identification authority of india" in line_lower
            ):
                if i + 1 < len(lines):
                    potential_name_line = lines[i + 1].strip()
                    if (
                        re.match(r"^[A-Za-z\s\.\-]+$", potential_name_line)
                        and len(potential_name_line.split()) >= 2
                        and 5 <= len(potential_name_line) <= 50
                        and not any(
                            keyword in potential_name_line.lower()
            ...
            # Look for explicit "Name:" field
                ...
                if match:
                    ...
                    if (
                        re.match(r"^[A-Za-z\s\.\-]+$", name_candidate)
                        and len(name_candidate.split()) >= 2
                        and 5 <= len(name_candidate) <= 50
                        and not any(
                            keyword in name_candidate.lower()
                            for keyword in garbage_keywords
                        )
                    ):
        ...
        for line in lines:
            ...
            if (
                ...
                and 2 <= len(line_clean.split()) <= 5
                and 5 <= len(line_clean) <= 50
                and not any(keyword in line_lower for keyword in garbage_keywords)
                and not (line_clean.isupper() and len(line_clean.split()) > 3)
            ):

                # Prefer CamelCase names
                if (
                    re.match(r"^[A-Z][a-z]+(?:[A-Z][a-z]+)+$", line_clean)
                    and len(line_clean) >= 8
                ):
                    return line_clean.title()

                if not name_candidate or (
                    len(line_clean) > len(name_candidate)
                    and "male" not in line_lower
                    and "female" not in line_lower
                ):
    ...
    def extract_relative_name_and_address(self, text):
        ...
        for rel_type_key, pattern in relative_patterns.items():
            ...
            if match:
                ...
                if (
                    ...
                    and len(candidate_name) <= 60
                    and re.match(r"^[A-Za-z\s\.\-]+$", candidate_name)
                    and not any(
                        keyword in candidate_name.lower()
                        for keyword in [
                            "dob",
                            "gender",
                            "aadhaar",
        ...
        if not relative_name:
            match = re.search(
                r"(Father)[:\s]*([A-Z][a-zA-Z\s\.\-]{2,60})", temp_text, re.I
            )
            if match:
                candidate_name = match.group(2).strip()
                if (
        ...
        if not relative_name:
            ...
            for line in lines:
                match = re.search(
                    r"^([A-Z][a-zA-Z\s\.\-]{3,60})\s*[,.]?\s*(?:H\.NO\.|HOUSE|VILLAGE|APARTMENT|STREET|ROAD|SECTOR|PHASE)",
                    line,
                    re.I,
                )
                if match:
        ...
        if relative_name and matched_string_to_remove:
            ...
            cleaned_text_for_address = re.sub(
                escaped_to_remove + r"[:,\s]*",
                " ",
                cleaned_text_for_address,
                flags=re.I,
            ).strip()

            # Remove relative name from address
            if relative_name.title() in cleaned_text_for_address:
                cleaned_text_for_address = cleaned_text_for_address.replace(
                    relative_name.title(), ""
                ).strip()
            ...
            cleaned_text_for_address = re.sub(
                r"\s{2,}", " ", cleaned_text_for_address
            ).strip()

        # Extract address
        lines_for_address = [
            line.strip()
            for line in cleaned_text_for_address.split("\n")
            if line.strip()
        ]
        address_candidates = []
        ...
        for line in lines_for_address:
            ...
            if (
                address_collection_started
                or re.search(r"\b\d{6}\b", line)
                or re.search(r"\bH\.NO\.\s*\-?\d+", line, re.I)
            ):
                is_garbage = any(
                    re.search(pattern, line_lower) for pattern in ignore_patterns
                )
                has_alphanumeric = (
                    len(re.findall(r"[A-Za-z0-9]", line)) >= len(line) * 0.3
                )
        ...
        final_address = re.sub(
            r"(?:PIN\s*CODE|PINCODE)[:\s]*", "", final_address, flags=re.I
        ).strip()
    ...
    def extract_front_details(self, image_path):
        ...
        details_region = self.crop_regions_of_interest(
            processed_full_img, "front_details"
        )
        text2_cropped = self.extract_text_with_paddle(details_region)

        number_region = self.crop_regions_of_interest(
            processed_full_img, "aadhaar_number"
        )
        ...
        results.append(
            {
                ...
                "Relative's Name": relative_name1,
                "Address": address1,
            }
        )
        ...
        results.append(
            {
                ...
                "Relative's Name": relative_name2,
                "Address": address2,
            }
        )
...
def main():
    ...
    front_img_path = (
        "/home/rishabh/openbharatocr/openbharatocr/ocr/Aadhar_sample/A1.jpeg"
    )
    back_img_path = (
        "/home/rishabh/openbharatocr/openbharatocr/ocr/Aadhar_sample/A2.jpeg"
    )

    try:
        details = aadhaar_ocr.extract_complete_aadhaar_details(
